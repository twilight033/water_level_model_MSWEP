...
# ── 故障统计（真实分布）配置 ────────────────────────────────────────────────────
# 来自 src/missing_period/analyze_fault_recovery.py 的输出
FAULT_STATS_DIR = Path(__file__).parent.parent / "missing_period" / "output"
FAULT_STATS_Q_CSV = FAULT_STATS_DIR / "fault_recovery_Q.csv"
FAULT_STATS_H_CSV = FAULT_STATS_DIR / "fault_recovery_H.csv"

# 数据时间分辨率（小时）。原始 fault_lengths_hours 以 1h 为单位，
# 而训练用的 DataFrame 已经重采样到 3h；采样后的小时数需除以该值换算成"步数"。
DATA_STEP_HOURS = 3


def load_fault_length_ecdf_hours(csv_path: Path) -> np.ndarray:
    """
    从故障统计 CSV 的 fault_lengths_hours 列加载全部段长样本（单位：小时），
    合并为一个大的 1D 数组，用于后续 bootstrap（等价于 ECDF 采样）。

    Parameters
    ----------
    csv_path : Path
        fault_recovery_Q.csv 或 fault_recovery_H.csv
    ...
    if "fault_lengths_hours" not in df.columns:
        raise ValueError(f"{csv_path} 缺少 fault_lengths_hours 列")
    ...
    if not pieces:
        raise ValueError(f"{csv_path} 中 fault_lengths_hours 全为空")

    all_lengths = np.concatenate(pieces)
    print(f"  已加载 {csv_path.name}: {len(all_lengths):,} 个故障段样本"
          f" (中位数 {np.median(all_lengths):.0f}h, "
          f"p95 {np.percentile(all_lengths, 95):.0f}h, "
          f"max {all_lengths.max()}h)")
    return all_lengths


def _hours_to_steps(hours: int, step_hours: int = DATA_STEP_HOURS) -> int:
    """将小时数换算成 3h 分辨率下的步数，四舍五入并至少占 1 步。"""
...
def create_missing_mask(data_df, missing_ratio, seed=42,
                        fault_lengths_hours=None):
    """
    段状标签缺失（基于真实故障段长度 ECDF 做 bootstrap 注入）。

    生成过程（对每个流域独立进行）：
      1. 统计原本有效点数 n_valid，目标新挖掉 n_inject = int(n_valid * missing_ratio)；
      2. 循环：
           - 在"当前仍有效"的位置里**均匀随机**挑一个起点（段的起点）；
           - 从 fault_lengths_hours 数组里 bootstrap 采一段长度（小时），
             换算成本分辨率下的"步数" gap_steps；
           - 将 [start, start + gap_steps) 范围内原本有效的点置 NaN，
             新增的 NaN 数累加到已挖掉计数；
         直到达到 n_inject 目标量（最后一段若超出会自然截断）。

    均匀随机挑起点 → 段与段之间的间距天然服从指数分布（泊松过程的等价定义），
    所以 λ 会被 n_inject / T 自动决定，不需要显式调用 rng.exponential。

    Parameters
    ----------
    data_df : pd.DataFrame
        原始数据（index 为时间、columns 为流域ID）
    missing_ratio : float
        额外注入比例（= 新挖掉点数 / 原有效点数）；0 表示不注入
    seed : int
        随机种子（仅作用于本函数的局部生成器）
    fault_lengths_hours : np.ndarray | None
        真实故障段长度样本数组（单位：小时）。
        missing_ratio>0 时必须传入。

    Returns
    -------
    pd.DataFrame
        带有段状人工缺失的数据（缺失部分设为 NaN）
    """
    if missing_ratio == 0.0:
        print(f"  保持数据完整（无人工缺失）")
        return data_df.copy()

    if fault_lengths_hours is None or len(fault_lengths_hours) == 0:
        raise ValueError("段状注入需要传入 fault_lengths_hours（来自 ECDF）")

    rng = np.random.default_rng(seed)
    masked_data = data_df.copy()
    original_valid = masked_data.notna()

    fault_lengths_hours = np.asarray(fault_lengths_hours)

    for col in masked_data.columns:
        valid_mask = original_valid[col].values
        valid_count = int(valid_mask.sum())
        if valid_count == 0:
            continue

        n_inject = int(valid_count * missing_ratio)
        if n_inject <= 0:
            continue

        col_idx = masked_data.columns.get_loc(col)
        n_rows = len(masked_data)
        # current_valid[i] 表示该流域 i 处是否仍为有效（会随注入逐步更新）
        current_valid = valid_mask.copy()
        injected = 0

        # 防止极端情况下的死循环（理论上不会，但保留上限）
        max_iters = n_inject * 10 + 1000
        it = 0

        while injected < n_inject and it < max_iters:
            it += 1
            # 在"当前仍有效"的位置里均匀挑一个起点
            valid_positions = np.flatnonzero(current_valid)
            if len(valid_positions) == 0:
                break
            start = int(rng.choice(valid_positions))

            # 从真实 ECDF 采段长（小时）→ 换算成本分辨率下的步数
            gap_hours = int(rng.choice(fault_lengths_hours))
            gap_steps = _hours_to_steps(gap_hours)

            end = min(start + gap_steps, n_rows)
            # 本段内原本有效、且当前还未被挖的位置 → 置 NaN
            newly_hidden = current_valid[start:end]
            k = int(newly_hidden.sum())
            if k <= 0:
                continue

            # 如果本段会超预算，随机截断到正好达标
            if injected + k > n_inject:
                extra = injected + k - n_inject
                # 在 [start, end) 里选择要保留的原有效点，数量 = extra
                seg_valid_pos = start + np.flatnonzero(newly_hidden)
                keep = rng.choice(seg_valid_pos, size=extra, replace=False)
                hide = np.setdiff1d(seg_valid_pos, keep, assume_unique=False)
            else:
                hide = start + np.flatnonzero(newly_hidden)

            masked_data.iloc[hide, col_idx] = np.nan
            current_valid[hide] = False
            injected += len(hide)

        # 按流域报告实际注入量（调试用，可以关掉）
        # actual = injected / valid_count if valid_count > 0 else 0.0
        # print(f"    流域 {col}: 有效点 {valid_count}，目标注入 {n_inject}，实际 {injected} ({actual:.2%})")

    original_valid_count = int(original_valid.sum().sum())
    final_valid_count = int(masked_data.notna().sum().sum())
    actual_missing_ratio = 1 - (final_valid_count / original_valid_count) \
        if original_valid_count > 0 else 0.0

    print(f"  原始有效数据点: {original_valid_count}")
    print(f"  人工隐藏后有效数据点: {final_valid_count}")
    print(f"  实际缺失比例: {actual_missing_ratio:.2%}"
          f"  (目标 ≈ {missing_ratio:.2%}，段状注入)")
...
def main(architecture="wl2d", output_root=None, model_seed=1234):
    ...
    if output_root is None and architecture == "parallel":
        output_root = Path(__file__).resolve().parents[2] / "results" / "parallel" / "segment"
    OUTPUT_ROOT = Path(output_root) if output_root is not None else Path("results") / "ablation_realistic_missing"
    MODEL_SAVE_PATH = str(OUTPUT_ROOT / "models")
    IMAGES_SAVE_PATH = str(OUTPUT_ROOT / "images")
    REPORTS_SAVE_PATH = str(OUTPUT_ROOT / "reports")
    LOGS_SAVE_PATH = str(OUTPUT_ROOT / "logs")
    for p in [MODEL_SAVE_PATH, IMAGES_SAVE_PATH, REPORTS_SAVE_PATH, LOGS_SAVE_PATH]:
        os.makedirs(p, exist_ok=True)
    print(f"\n实验输出目录: {OUTPUT_ROOT}")

    # 加载真实故障段长度样本（用于段状注入的 ECDF）
    print("\n正在加载真实故障段长度（Q / H）...")
    flow_fault_lengths = load_fault_length_ecdf_hours(FAULT_STATS_Q_CSV)
    waterlevel_fault_lengths = load_fault_length_ecdf_hours(FAULT_STATS_H_CSV)
    ...
    experiment_kwargs = {
        ...
        'waterlevel_fault_lengths': waterlevel_fault_lengths,
    }
    ...
    for exp_idx, (flow_missing_ratio, waterlevel_missing_ratio, exp_name) in enumerate(experiments):
        ...
        for seed_idx, mask_seed in enumerate(repeat_seeds):
            ...
            # 保存单次模型
            model_path = os.path.join(MODEL_SAVE_PATH, f'{architecture}_realistic_{run_name}.pth')
            torch.save({
    ...
    raw_csv = os.path.join(REPORTS_SAVE_PATH, f"{architecture}_realistic_raw_results.csv")
    ...
    agg_csv = os.path.join(REPORTS_SAVE_PATH, f"{architecture}_realistic_aggregated_results.csv")
    ...
    comparison_file = os.path.join(
        IMAGES_SAVE_PATH, f"{architecture}_realistic_comparison.png"
    )
