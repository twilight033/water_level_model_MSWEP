# -*- coding: utf-8 -*-
"""
绘制单任务 LSTM 模型架构图（径流基线，时序展开风格）。

对应模型：src/training/single_task_lstm - 早停+正则.py 中的 SingleTaskLSTM
- 共享 2 层 LSTM（hidden=64），单向 many-to-one（用末步隐状态预测）
- 单一输出头 fc(64→1) 预测径流 Q（target_type="flow"）
- 训练：MSELoss + Adam + 权重衰减 1e-5（L2 正则）+ 早停（patience=5）

与 WL2D 多任务模型共用同一套视觉风格，作为对照基线：只有一个输出头，
没有水位头、没有水位→径流级联通路。

产出：outputs/ppt-assets/single_task_architecture.png
"""
...
# ==================== 配色 ====================
C_INPUT = "#D9EAD3"; C_INPUT_E = "#6AA84F"
C_LSTM = "#F4CCCC"; C_LSTM_E = "#CC6666"
C_HEAD = "#CFE2F3"; C_HEAD_E = "#3D85C6"
C_SRC = "#D9D9D9"; C_SRC_E = "#999999"
C_ATTR = "#EFEFEF"; C_ATTR_E = "#999999"
C_LOSS = "#FCE5CD"; C_LOSS_E = "#E69138"
C_OBS = "#FFFFFF"; C_OBS_E = "#666666"
C_ARROW = "#444444"
...
def main():
    fig, ax = plt.subplots(figsize=(13.0, 9.1))
    ax.set_xlim(0, 13.0)
    ax.set_ylim(0, 9.6)
    ax.axis("off")
    ...
    x_head = xT   # 单一输出头，居于末步 LSTM 正上方

    # ==================== 标题 ====================
    ax.text(6.4, 9.32, "多流域 单任务 LSTM（径流基线模型）",
            ha="center", va="center", fontsize=FS_TITLE, fontweight="bold")
    ax.text(6.0, 8.88, "共享 LSTM 编码器 · 单一径流输出头 · MSE 损失 + L2 正则 + 早停",
    ...
    # h_T：末步 LSTM 隐状态 -> 输出头
    draw_arrow(ax, (x_head, y_top_lstm), (x_head, y_box - 0.41), lw=2.2)
    ax.text(x_head + 0.22, (y_top_lstm + y_box - 0.41) / 2, r"$h_T$",
            ha="left", va="center", fontsize=FS_ANNO + 1, color="#333333")

    # 输出头 -> 径流预测
    draw_box(ax, x_head, y_box, 1.95, 0.82, "径流输出头\nfc (64→1)", C_HEAD, C_HEAD_E, fontsize=FS_BOX)
    draw_arrow(ax, (x_head, y_box + 0.41), (x_head, y_pred - r_pred))
    draw_circle(ax, x_head, y_pred, r_pred, r"$\hat{Q}_T$", C_LSTM, C_LSTM_E,
                fontsize=FS_VAR, fontweight="bold")
    ax.text(x_head - r_pred - 0.14, y_pred, "径流\n预测", ha="right", va="center",
            fontsize=FS_ANNO, color="#a03a3a")

    # ==================== 6. 观测与损失 ====================
    draw_circle(ax, x_head, y_obs, r_obs, r"$Q_{obs}$", C_OBS, C_OBS_E, fontsize=FS_OBS)
    draw_arrow(ax, (x_head, y_pred + r_pred), (x_head, y_obs - r_obs),
               color=C_LOSS_E, lw=2.1, style="<|-|>")
    ax.text(x_head - r_pred - 0.14, (y_pred + y_obs) / 2 + 0.18, r"$loss_Q$",
            ha="right", va="center", fontsize=FS_ANNO + 1, color=C_LOSS_E)

    # loss / 训练 公式框（右侧）
    loss_cx, loss_cy = 10.75, 7.75
    draw_box(ax, loss_cx, loss_cy, 3.9, 1.2,
             r"$loss = \mathrm{MSE}(\hat{Q}_T,\ Q_{obs})$"
             "\nAdam · L2 正则 (wd=1e-5) · 早停",
             C_LOSS, C_LOSS_E, fontsize=FS_LOSS)
    draw_arrow(ax, (x_head + r_obs + 0.05, y_obs), (loss_cx - 1.98, loss_cy + 0.05),
               color=C_LOSS_E, lw=1.8, connectionstyle="arc3,rad=-0.15")

    # 单任务基线说明（填补左上空白）
    ax.text(3.3, 6.85, "单任务基线\n只预测径流一个目标\n（无水位头、无级联通路）",
            ha="center", va="center", fontsize=FS_SMALL + 0.5, color="#777777")

    # ==================== 保存 ====================
    out_dir = os.path.join("outputs", "ppt-assets")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "single_task_architecture.png")
