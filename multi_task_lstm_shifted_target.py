# multi_task_lstm_shifted_target.py
#
# 修改说明（相对于 multi_task_lstm.py）：
# MSWEP 的时间戳约定为"向前累计"：t 时刻的值代表 t → t+3h 的累计降雨。
# 原版代码中，预测目标为序列末尾时刻 t_end = t₀ + (L-1)*3h，
# 而输入序列最后一步正是 MSWEP[t_end]，即 t_end → t_end+3h 的未来降雨，
# 形成数据泄露。
#
# 本脚本采用方案 A：将预测目标后移一步，改为 t_end = t₀ + L*3h，
# 使输入序列 MSWEP[t₀..t₀+(L-1)*3h] 覆盖的降雨全部发生在目标时刻之前，
# 消除泄露。模型的物理含义变为：用过去 L 个 3h 的气象条件预测下一个 3h 末的 Q/h。
#
# 仅改动以下位置，其余代码与原版完全一致：
#   __getitem__          : end_time 计算（2处）
#   _create_lookup_table : end_time 计算（2处）
#   可视化 start_date    : 偏移量（1处）
#   模型保存文件名        : (1处)
...
class MultiTaskDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        # 获取两个目标输出（序列末尾后移一步的时刻）
        # [shifted_target] MSWEP[t] 代表 t→t+3h 的累计降雨（向前约定），
        # 因此输入序列最后一步 MSWEP[t₀+(L-1)*3h] 覆盖至 t₀+L*3h，
        # 预测目标设为 t₀ + L*3h 以避免使用未来降雨。
        target_time_idx = time_idx
        if target_time_idx in self.data_flow.index:
            # 目标时刻 = 序列起点 + L 步（比原版多一步）
    ...
    def _create_lookup_table(self):
        ...
        for basin in tqdm(self.basins, desc=f"创建 {self.loader_type} 索引表", disable=False):
            ...
            for ft in forcing_times:
                # [shifted_target] 目标时刻后移一步：t₀ + L*3h
...
if __name__ == "__main__":
    ...
    # 注意：使用3小时分辨率（freq='3H'）
    # [shifted_target] 目标时刻后移一步，所以 start_date 也相应后移
    start_date = pd.to_datetime(ds_test.dates[0], format="%Y-%m-%d") + pd.Timedelta(
        hours=sequence_length * 3  # 3小时分辨率，后移一步
    )
    ...
    # ==================== 10. 保存模型 ====================
    print("\n正在保存模型...")
    model_path = os.path.join(MODEL_SAVE_PATH, 'multi_task_lstm_shifted_target.pth')
    torch.save({
